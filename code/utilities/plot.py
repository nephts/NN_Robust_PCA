...
def plot_matrices(M_test, U_pred=None, L_pred=None, vmin=None, vmax=None):
    ...
    im = axes[0].imshow(M_test, vmin=vmin, vmax=vmax)
    axes[0].set_ylabel('M    ', fontsize='x-large', rotation=0)
    fig.colorbar(im, ax=axes[0])

    im = axes[1].imshow(L_pred, vmin=vmin, vmax=vmax)
    axes[1].set_ylabel('L    ', fontsize='x-large', rotation=0)
    fig.colorbar(im, ax=axes[1])
    
    im = axes[2].imshow(S_pred, vmin=vmin, vmax=vmax)
...
def test_UV(U_set, V_set, M_set, net, n):
    ...
    plot_matrices(M_set[index], L_pred=L_pred, vmin=-2, vmax=2)
